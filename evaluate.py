...
def main() -> None:
    ...
    if args.markdown:
        ...
        print("\n| Dificultad | Exactitud | n |")
        print("|---|---|---|")
        for level in ("clean", "scanned", "photo", "hard"):
            if by_difficulty[level]:
                print(f"| {level} | {pct(by_difficulty[level]):.0f}% | {len(by_difficulty[level])} |")
    else:
        print(f"\nEvaluado sobre {evaluated} documentos, {len(overall)} campos\n")
        print(f"{'campo':16} {'exactitud':>10} {'marcados':>10}   n")
        print("-" * 46)
        for name in COMPARED_FIELDS:
            if by_field[name]:
                print(f"{name:16} {pct(by_field[name]):9.0f}% {pct(review_flagged[name]):9.0f}% "
                      f"{len(by_field[name]):4}")
        print("-" * 46)
        print(f"{'GLOBAL':16} {pct(overall):9.1f}%  {len(overall):>13}\n")

        print(f"{'dificultad':16} {'exactitud':>10}   n")
        print("-" * 34)
        for level in ("clean", "scanned", "photo", "hard"):
            if by_difficulty[level]:
                print(f"{level:16} {pct(by_difficulty[level]):9.0f}% {len(by_difficulty[level]):4}")
